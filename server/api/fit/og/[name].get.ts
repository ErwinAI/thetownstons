...
export default defineEventHandler(async (event) => {
  ...
  if (!snap?.body) {
    ...
    if (fallback) return sendPng(event, fallback, 'public, max-age=3600')
    throw createError({ statusCode: 404, statusMessage: 'No sheet to preview' })
  }
  ...
  if (stored?.bytes) {
    return sendPng(event, stored.bytes, 'public, max-age=86400, immutable')
  }
  ...
  if (made.bytes) {
    return sendPng(event, made.bytes, 'public, max-age=3600')
  }
  const fallback = await loadDefaultOg()
  if (fallback) return sendPng(event, fallback, 'public, max-age=300')
